...
#[test]
fn ipc_boundary_registers_reviewed_persistence_commands_and_removes_greet() -> TestResult {
...
fn parse_generate_handler_commands(handler_body: &str) -> Vec<String> {
    handler_body
        .split(',')
        .map(str::trim)
...
fn assert_reviewed_db_command_exposure(
    ...
    let granted_permission_ids = capability_paths
        .iter()
        .map(|path| parse_json_file(path))
        .collect::<Result<Vec<_>, _>>()?
        .iter()
        .flat_map(collect_capability_permission_ids)
        .collect::<Vec<_>>();

    let has_explicit_acl = ["allow-db-execute", "allow-db-transaction"]
        .into_iter()
        .all(|required| {
            granted_permission_ids
                .iter()
                .any(|permission_id| permission_id == required)
        });

    assert_generated_permission_allows(
        &repo_root.join("src-tauri/permissions/autogenerated/db_execute.toml"),
        "allow-db-execute",
        "db_execute",
    )?;
    assert_generated_permission_allows(
        &repo_root.join("src-tauri/permissions/autogenerated/db_transaction.toml"),
...
fn collect_capability_permission_ids(capability_json: &Value) -> Vec<String> {
    let mut permission_ids = Vec::new();
    collect_permission_ids_from_value(capability_json.get("permissions"), &mut permission_ids);
    permission_ids
}

fn collect_permission_ids_from_value(value: Option<&Value>, permission_ids: &mut Vec<String>) {
    match value {
        Some(Value::String(value)) => permission_ids.push(value.to_string()),
        Some(Value::Array(values)) => {
            for value in values {
                collect_permission_ids_from_value(Some(value), permission_ids);
            }
        }
        Some(Value::Object(object)) => {
            collect_permission_ids_from_value(object.get("identifier"), permission_ids);
            collect_permission_ids_from_value(object.get("id"), permission_ids);
        }
        _ => {}
    }
}

fn assert_generated_permission_allows(
    path: &Path,
    expected_identifier: &str,
    expected_command: &str,
) -> TestResult {
    let permissions = parse_generated_permissions(path)?;
    let allow_permission = permissions
        .iter()
        .find(|permission| permission.identifier == expected_identifier)
        .unwrap_or_else(|| {
            panic!(
                "generated permission {} must contain identifier {}",
                path.display(),
                expected_identifier
            )
        });
    ...
    assert_eq!(
        allow_permission.commands_allow,
        vec![expected_command.to_string()],
        "generated permission {} must map {} to commands.allow = {:?}",
        path.display(),
        expected_identifier,
        [expected_command]
    );
...
fn parse_generated_permissions(path: &Path) -> TestResult<Vec<GeneratedPermission>> {
    let contents = read_to_string(path)?;
    let mut permissions = Vec::new();
    let mut current: Option<GeneratedPermission> = None;

    for line in contents.lines() {
        let line = line.trim();
        if line == "[[permission]]" {
            if let Some(permission) = current.take() {
                permissions.push(permission);
            }
            current = Some(GeneratedPermission::default());
            continue;
        }
        ...
        if let Some(raw_identifier) = line.strip_prefix("identifier = ") {
            permission.identifier = parse_toml_string(raw_identifier)?;
        } else if let Some(raw_commands) = line.strip_prefix("commands.allow = ") {
            permission.commands_allow = parse_toml_string_array(raw_commands)?;
        }
    }

    if let Some(permission) = current {
        permissions.push(permission);
    }
...
fn parse_toml_string(raw: &str) -> TestResult<String> {
    let raw = raw.trim();
    if !raw.starts_with('"') || !raw.ends_with('"') || raw.len() < 2 {
        return Err(format!("expected TOML string, got {raw:?}").into());
    }

    Ok(raw[1..raw.len() - 1].to_string())
}

fn parse_toml_string_array(raw: &str) -> TestResult<Vec<String>> {
    let raw = raw.trim();
    if !raw.starts_with('[') || !raw.ends_with(']') {
        return Err(format!("expected TOML string array, got {raw:?}").into());
    }
