...
## TASK-043 ML And AI Context Panels Guidance

TASK-043 coverage lives primarily in `src/test/ml-ai-context-projections.test.ts` and `src/test/ml-ai-context-panels.test.tsx`, with supporting shell, host, ML, and AI assertions in `src/test/app-shell-boundary.test.ts`, `src/test/view-slot-hosts.test.tsx`, `src/test/ml-plugin-baseline-predictions.test.tsx`, and `src/test/ai-plugin-provider-abstraction.test.tsx`. It proves the page-only right context panel without adding package/native/Tauri/Rust/IPC/capability/permission/schema/release drift or live provider execution.

- Projection coverage should assert exact shell-owned current-page DTOs from public runtime pages/metadata/events, current-page and direct-child scoping, Task estimate/status metadata, Tag tags metadata, Timer segment/note evidence, missing/archived/malformed current-page fail-closed behavior, wrong-owner/accessor/prototype/symbol/secret-shaped filtering, ML caps at 1,000 rows, AI caps at 100 rows, current-page AI body text caps at 50,000 chars, and `ai.explain-prediction` appears only after a valid current-page `ml.remaining-time-prediction`.
- Panel coverage should use React Testing Library plus `userEvent.setup()` for opening/closing the `Page context` complementary panel, preserving the editable Markdown workspace, focus return, page-route gating, no ML/AI Drawer routes, tab switching, and exact `ViewHost` ids `ml.prediction-panel`, `ai.suggestion-panel`, and `ai.review-panel`.
- ML command coverage should assert active owned `ml.run-prediction` execution through Command Registry with the exact current-page projection payload, valid result rendering through `ml.prediction-panel`, visible partial/unavailable/error states, and no unrelated page bodies, provider settings, secrets, raw runtime handles, or raw errors in the UI.
- AI command coverage should assert only `ai.suggest-tags`, `ai.suggest-due-date`, `ai.generate-subtasks`, and gated `ai.explain-prediction` are exposed; commands execute only active `ai`-owned descriptors; output remains advisory; errors are redacted; and runtime pages, metadata, events, filters, sibling plugin data, settings, secrets, provider configuration, and durable AI metadata/events are not mutated by shell integration.
- Stale async coverage should assert page switches or closes invalidate older ML/AI resolves or rejects, so old page text, predictions, provider errors, paths, and secrets cannot appear after navigation.
- Static guards should keep app-shell/projection code free of private ML/AI plugin imports, broad `page.sidebar.panel` mounting, package/lockfile/Cargo/Tauri/capability/permission/IPC/schema/release drift, live provider/network/storage/worker/secret/settings surfaces, OpenAI implementation details, native imports, execution sinks, stale MUI/React/testing APIs, and focused/skipped tests.

Focused TASK-043 validation:

```bash
bun run test:frontend -- src/test/ml-ai-context-projections.test.ts src/test/ml-ai-context-panels.test.tsx src/test/app-shell-boundary.test.ts src/test/view-slot-hosts.test.tsx src/test/ml-plugin-baseline-predictions.test.tsx src/test/ai-plugin-provider-abstraction.test.tsx
bun run typecheck
bun run lint
git diff --check
```
