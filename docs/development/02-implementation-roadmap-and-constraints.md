...
点击 A 进入 A 页面、A 页面写 `- [ ] B` 后点击 B 进入 B 页面，是 TASK-019 当前显式 click/open 行为。点击 checkbox 切换 source task line 状态并写 metadata/event 是 TASK-020 当前行为。TASK-022 当前交付 All Tasks / Today filters 的 data-only execution 和 registered `page.list` rendering slice。TASK-023 through TASK-032 已交付 reusable MetadataBar slice、Timer Start control、global active timer bar、Time Segment events、Time Segment Note Markdown Pages、`page.timeline` segment/note slot、Calendar day/week normalized segment rendering baseline、Habit commands/filters、Heatmap normalized DTO view baseline、Stats normalized aggregation baseline、Chart accessible DTO view baseline、ML deterministic prediction DTO baseline、AI provider-abstraction/advisory command baseline 和 Sync Plugin skeleton。TASK-038 当前已交付 app-shell Drawer page/saved-filter navigation：Home/recent pages 经 `ViewHost` editor，Inbox/Today/All Tasks/public saved filters 经 public `FilterDefinition`、`executeFilterQuery`、`ViewHost` 和 `SlotHost`，并只传 `{ routeToken, title }` DTO。TASK-039 当前把 page-route `MetadataBar`、page-route `page.timeline` 和 `global.floating` active timer bar 接入 production app shell；metadata/timeline 只出现在 page routes，floating timer 通过 MUI `Portal` 渲染且只获得 timer-owned Pause / Resume / Stop facade。TASK-042 当前把 Calendar day/week 和 Reports routes 接入 Drawer，用 public runtime pages/events/metadata 生成 bounded route projections，Calendar 只通过 narrow `calendar.open-time-segment` bridge 打开当前 projection 内 segment，Reports 运行 active `stats.run-aggregation` 并通过 `chart.bar` 渲染返回 DTO。保存后自动扫描/索引、broader global/persistent saved-filter navigation、Habit/Heatmap/AI/Sync route、完整 metadata editor registry、Task checkbox 自动桥接 Habit、Timer metadata totals、persistent Calendar/Reports dashboards or cross-plugin feed/query facade、trusted/persistent ML feed integration、AI settings UI/secret storage/live provider execution、Sync network/native transport、rich editor behavior 和 native/package surfaces 仍是后续范围。
...
No save-time scan, background indexer, rich inline token UI, autocomplete, or global metadata bar. TASK-038 later adds current app-shell saved-filter routes for public filters; broader global/persistent filter navigation remains future scope.
...
`MetadataBar` 组合 plugin-owned `page.header.metadata` slot contributions，并用 active Plugin Host manifest ownership data 过滤 trusted field descriptors/values。Manifest `metadataFields` 仍是 inert descriptors / reservation inputs，不是 executable renderer/editor declarations。TASK-039 后，production app shell 在 page routes 上默认挂载 public `metadata-ui` `MetadataBar`。完整 metadata renderer/editor registry、date picker、estimate editor、完整 tag picker polish、save-time scanning/indexing 和 Timer tracked-total metadata 仍是后续范围；Time Segment timeline/note UI 只通过 TASK-039 的 page-route `page.timeline` SlotHost 挂载。
...
Still deferred after TASK-026/TASK-030, except for TASK-042's later transient Calendar/Reports routes: Timer metadata totals, broad cross-plugin event read/query facade, persistent Calendar/Reports feeds, trusted/persistent ML feed integration, Recently Worked / Unnoted Sessions saved filters, manual segment editing, calendar drag/drop, native persistence/schema, and Tauri/package/Rust/native changes.
...
persistent Calendar dashboards/feeds beyond TASK-042 day/week route projections
drag/drop editing
...
TASK-030 不实现 executable AlgorithmRegistry、runtime algorithm handler、trusted query/feed facade、durable prediction metadata/events from caller-provided projections、model refresh/training/background jobs、best work time detection、task ranking、similar task clustering、recommendation、ML-native explanation beyond TASK-031 `ai.explain-prediction` advisory command、app-shell polish 或 native/package/Rust/schema/Tauri capability changes。
...
```text
plugin id ai in BUILT_IN_PLUGINS
provider id openai
default model guidance gpt-5.5
provider boundary under src/plugins/ai/**
runtime commands ai.cleanup-inbox / ai.turn-text-into-task / ai.suggest-tags / ai.suggest-due-date
runtime commands ai.generate-subtasks / ai.generate-filter / ai.summarize-time-notes / ai.generate-weekly-review / ai.explain-prediction
metadata descriptors ai.summary / ai.suggestedTags / ai.suggestedEstimate
event descriptors ai.suggestion-generated / ai.summary-generated
settings descriptor ai.provider-settings
views ai.suggestion-panel / ai.review-panel
OpenAI Responses-style request shape: instructions, string input, store false, text.format strict json_schema
mocked/injected provider and transport tests
```

All command ids are kebab-case only; old underscore ids such as `ai.cleanup_inbox`, `ai.generate_subtasks`, `ai.suggestion_panel`, and `ai.summary_generated` are stale docs and not aliases.

AI commands currently consume exact bounded caller-provided projections and return advisory DTOs only. They do not mutate Markdown Pages, metadata, events, filters, settings, or sibling plugin data; they do not persist AI metadata/events; and they do not read sibling plugin private stores/facades. Core remains AI/provider/model/prompt-free.

The OpenAI boundary is plugin-owned and test-injected. TASK-031 does not add an OpenAI SDK, live network calls, raw `fetch`, native HTTP, package dependencies, Tauri permissions/capabilities, Rust commands, persistence schema, keychain, or secret storage. Provider settings are currently AI-plugin-owned injectable runtime/test state plus an inert `ai.provider-settings` descriptor; persistent plugin settings, settings UI, OS keychain/secret storage, native HTTP transport, live provider execution, app-shell acceptance UX, stricter raw Responses missing-status parsing, exact public wording preservation for `persist*`, and `generate-filter` parity with Core `neq` / `exists` semantics remain deferred.
...
```text
plugin id sync in BUILT_IN_PLUGINS
no runtime commands, views, settings panels, or transport
syncable unit descriptors for Markdown Page / Metadata / Event / Filter / Plugin Settings
deterministic DTO serializers for those units
local plugin indexes marked rebuildable and not syncable
mutable-unit divergence -> manual-resolution-required
event append-only union / duplicate dedupe / same-id-content conflict
event conflict helper rejects stale/mismatched/non-plain/malformed event units and requires exact descriptor-safe event DTO keys with snapshot.id == syncKey.id
tombstones, deletes, and conflict UI deferred
```

Plugin Settings are represented only as caller-provided DTO snapshots with `pluginId`, `key`, `state`, and `updatedAt`; top-level or nested secret/auth/credential/remote-endpoint-like keys are rejected. Future settings sync should use explicit allowlists and keychain separation for secrets or remote credentials. TASK-032 does not add settings UI, persistent plugin settings, remote endpoint settings, secret storage/keychain, native sync transport, network calls, package/Cargo changes, Rust commands, schema changes, or Tauri permissions/capabilities. No network/native sync may be enabled without explicit settings and security review.
...
TASK-029 当前 Quick Capture 和 Search 都是 TypeScript built-in plugin baseline。Quick Capture 注册 `quick-capture.open`、`quick-capture.save`、`quick-capture.save-and-open`、`quick-capture.modal`、`quick-capture.mobile-input`、`quick-capture.unprocessed` 和 `quick-capture.filter.inbox`；Search 注册 `search.query` 和 `search.results`，并在命令执行时按需扫描未 archived pages。TASK-031 当前 AI Plugin 也是 TypeScript built-in plugin baseline：注册 `ai`、canonical kebab-case AI commands、`ai.suggestion-panel` / `ai.review-panel` views、AI metadata/event descriptors 和 inert `ai.provider-settings` descriptor，并通过 plugin-owned `openai` provider boundary 生成 advisory DTO。TASK-032 当前 Sync Plugin 也是 TypeScript built-in plugin skeleton：注册 built-in plugin id `sync`，但没有 runtime commands/views/settings panels/transport，只导出 syncable unit DTO descriptors/serializers、rebuildable local plugin-index policy 和 conflict policy。这个 slice 没有 package/native/Tauri/Rust/schema/capability 变更，也没有 global shortcut、mobile toolbar mounting、persistent Search indexing、search indexer worker、SQLite/FTS、AI secret storage/settings UI/live provider execution 或 Sync network/native transport。
...
TASK-019 可以用同样 click/open 流程创建或打开子任务页，因此任务可以显式无限嵌套。保存后自动扫描、Tag/Timer/UI 自动刷新、broader global/persistent filter route 和 rich editor 行为仍属于后续任务。
