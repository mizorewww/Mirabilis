...
TASK-039 mounts that boundary in the production app shell only for trusted page routes, using the public `metadata-ui` `MetadataBar` export. The same task mounts `page.timeline` through `SlotHost` with only `{ page: { id, title } }`, and mounts `global.floating` through MUI `Portal` as React-owned portal children. The floating surface does not receive page props or raw `runtime.commands`; the reviewed Timer path exposes only a timer-owned `pause` / `resume` / `stop` facade that dispatches exact `{}` payloads.
...
- TASK-025 adds an internal non-enumerable plugin-scoped command executor to command-time `PluginContext` for Timer-owned slot UI. It authorizes by the registered command descriptor owner (`descriptor.pluginId === pluginId`) before dispatch, not by command id prefix. Known residual P2: the internal channel is keyed by globally discoverable `Symbol.for("mirabilis.internal.pluginScopedCommandExecutor")` and is duplicated between Plugin Host and Timer; descriptor-owner checks protect execution, but a future API cleanup should replace this hidden channel with a controlled internal contract.
