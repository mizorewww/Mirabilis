...
`BUILT_IN_PLUGINS` 在 TASK-021 后包含内置 `MarkdownEditorPlugin`、`TaskPlugin` 和 `TagPlugin`。Quick capture、search、timer、calendar 等其他具体业务内置插件仍属于后续插件任务。`loadBuiltInPlugins(BUILT_IN_PLUGINS)` 接收的是 App 启动时显式传入的插件对象，不表示文件系统发现、动态 import 或 native 插件加载。
