...
TASK-025 也注册 `timer.global-active-bar` 到 `global.floating`，显示 active page title、elapsed time 和 Pause / Resume / Stop controls。该 bar 读取 Timer Plugin registration-scoped in-memory active timer state；Timer finalization 会追加 event-backed Time Segment。TASK-026 的 Calendar day/week view 可以在调用方显式挂载并传入 `calendar.time-segments` DTO 时渲染 Timer segment projection。Calendar app-shell route/feed、Stats/ML integration、Timer metadata totals、Recently Worked、Unnoted Sessions、manual segment editing、calendar drag/drop 和 native persistence/schema/Tauri/package/Rust changes 仍是后续范围。
