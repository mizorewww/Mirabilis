...
- Mill the 2nd completed and was closed after adding the Markdown Editor Plugin shell under `src/plugins/markdown-editor/**`, built-in registration, runtime markdown extension collection, safe PluginHost plugin metadata listing, and narrow `pageFacade` load/save support. It also added a test-only harness correction for `userEvent.keyboard()` literal Markdown brackets.
