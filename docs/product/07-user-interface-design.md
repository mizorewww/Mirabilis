...
The first implementation task after this design document was TASK-035: add the MUI substrate and first app shell frame. TASK-036 added the generic `ViewHost` / `SlotHost` boundary. TASK-037 mounted the Home workspace editor through that boundary. TASK-038 added sidebar page and saved-filter navigation. TASK-039 mounted page metadata, page timeline, and global floating timer slots. TASK-040 added app-shell Command Palette and Quick Capture dialogs. TASK-041 added app-shell Search dialog/results routing. Later UI work continues with remaining route projections, panels, Settings/Sync placeholders, native/global capture entry points, mobile capture toolbar, persistent Search index/worker/SQLite FTS/ranking, native/global Search shortcuts, and responsive polish.
