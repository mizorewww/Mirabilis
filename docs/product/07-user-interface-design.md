...
This document defines the first complete Mirabilis UI design target before the MUI scaffold is implemented. It is a product and architecture design document, not a claim that the UI already exists.

## Diagnosis And Current Gap

TASK-001 through TASK-033 delivered the runtime substrate, built-in plugin behavior, persistence boundary, and local release gate. The user-visible app still needs a real desktop workspace: the startup experience is not yet the dense Markdown-first workbench described by the product docs.

The gap is not a missing business feature in Core. The gap is composition:

- App Shell does not yet mount a full product frame around the existing runtime.
...
The first implementation task after this document is TASK-035: add the MUI substrate and first app shell frame.
