...
- Future route surfaces, overlays, contextual panels, and broad projections should reuse the delivered MUI design system and TASK-045 accessibility conventions instead of inventing separate shell behavior.
...
| Responsive and accessibility | TASK-045 delivers MUI breakpoint-driven desktop/narrow shell behavior: permanent desktop navigation, temporary narrow navigation with close/focus return, desktop `Page context` as named `complementary`, narrow `Page context` as named modal `Dialog`, keyboard-reachable dialogs, named landmarks, labelled controls, focus return, and role/name based test coverage. | Tauri WebDriver or app-level E2E can follow after the route surface is stable. Native mobile integration remains out of scope. |
...
- The right context panel is optional and must never block the editor on desktop. On narrow layouts TASK-045 renders it as a named modal MUI `Dialog`, not a Drawer route.
...
| `ContextPanel` | `Box`, `Dialog` on narrow, `Tabs` or `ToggleButtonGroup` if needed | Current page projection DTOs for ML/AI panels. | TASK-043 page-only panel renders exact `ViewHost` ids `ml.prediction-panel`, `ai.suggestion-panel`, and `ai.review-panel`; TASK-045 keeps desktop as `complementary` and narrow as a named modal Dialog with Escape close, focus containment, and launcher focus return. AI commands stay advisory and allowlisted. |
...
2. Shell restores or selects the durable Home Markdown Page from shell-owned Core metadata, or creates one if no valid Home identity exists.
...
2. Shell stores the selected page route in shell state and persists the durable route identity through Core metadata.
...
TASK-038 left broad global route databases, Event/plugin-index `within` execution, save-time indexing, and arbitrary plugin view routes without explicit DTO designs for later tasks. TASK-047 now covers active public filter route restoration by persisting filter id, role, and optional route token through shell-owned Core metadata. TASK-042 has delivered the transient app-shell Calendar and Reports route projections; persistent feeds, broad query facades, global route databases, arbitrary plugin route DTOs, Search FTS, and backend/native routing remain separate future work.

### Durable Navigation Route State

TASK-047 delivers the first durable navigation layer on top of the existing Core metadata persistence:

1. The app shell writes a JSON Core metadata record on the durable Home page with namespace `app-shell.navigation`, key `route-state`, and owner/source plugin id `app-shell`.
2. The persisted value is versioned and contains only `version`, `homePageId`, the active page or filter route id/role, an optional filter route token, and capped recent page IDs.
3. Startup restoration reuses the durable Home identity when that page is still available and non-archived; otherwise it falls back to a safe Home page.
4. Page routes restore only when the page still exists and is not archived. Filter routes restore only after the filter source plugin, target view, plugin ownership data, and metadata-owner reservations are still trusted.
5. Malformed, stale, missing, archived, or wrong-owner records fail closed to safe Home or generic unavailable route state without rendering raw route DTO data.
6. TASK-047 does not add a Tauri Store, browser storage, IPC command, permission, schema, package, Cargo, Rust, native, global route database, arbitrary plugin route DTO system, Search FTS, or backend/native routing surface.
...
1. On a trusted page route, the user opens `Context Panel` from the top bar. On desktop the panel is a named `complementary` aside next to the Markdown workspace, not a Drawer route, and closing it returns focus to the launcher. On narrow layouts TASK-045 renders the same page context surface as a named modal MUI `Dialog` with Escape close, focus containment, editor preservation behind the dialog, and launcher focus return.
...
- Tablet/narrow desktop: collapsible left Drawer, central workspace, right panel opens as a named modal Dialog.
- Mobile-width desktop window: top bar stays compact, left Drawer is temporary, right context panel is hidden behind a button and opens as a Dialog, dialogs become full-screen where appropriate.
...
- TASK-045 narrow state starts with workspace navigation closed, closes temporary navigation after route selection, and returns focus to the navigation launcher. Desktop keeps navigation visible and exposes `Page context` as `complementary`; narrow exposes `Page context` as a named modal Dialog.
