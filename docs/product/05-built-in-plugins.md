...
Persistent Calendar feed/dashboard surfaces and direct cross-plugin read facade
Persistent Timer-to-Stats feed indexes and saved Stats routes beyond TASK-042 Reports
...
TASK-042 adds an app-shell Calendar route consumer without changing Calendar Plugin ownership. The shell snapshots public runtime pages/events/metadata, derives bounded `calendar.time-segments` rows from trusted Timer segment events joined to active non-archived pages, and mounts `calendar.day` / `calendar.week` through `ViewHost`. The route caps visible projected segments at 1,000 and reports partial status when it truncates. The route-owned command bridge delegates only `calendar.open-time-segment({ segmentId, pageId })` for a segment pair present in the current projection; it is not a raw command registry or generic command facade.
...
- The test harness and TASK-042 App Shell route may normalize public Timer `time_segment_created` events into `calendar.time-segments` DTOs, but that normalization is caller/view-host behavior.
...
Stats 消费调用方或 view host 传入的 normalized DTO。这些 DTO 可以来自公开的 plugin output、event 或 metadata 投影，但 Stats 不直接读取 Timer/Habit/Task/Tag internals，也不通过私有 store 查询其他插件数据。TASK-042 的 Reports route 是当前 app-shell 调用方：它从 public runtime pages/events/metadata 构造 bounded Stats input，默认执行 `stats.sum-time-by-page`，通过 active stats-owned `stats.run-aggregation` command 运行聚合，并把返回的 Chart-compatible DTO 交给 `chart.bar` / `ViewHost`。
...
TASK-042 Reports projection bounds Chart-compatible `stats.sum-time-by-page` and `stats.sum-time-by-tag` category outputs to 200 categories and surfaces partial status when rows are omitted. For tag reports, segment `tagIds` are limited to the emitted tag rows so Chart and Stats inputs stay aligned. Habit completion events, habit summaries, and Timer note rows also report partial status when they exceed the Stats input cap; missing public task estimate input remains partial rather than silently complete.

### 20.2 Chart Plugin 注册图表视图

TASK-028 当前注册内置 `chart` plugin。Chart 渲染 generic DTO，不查询 Stats internals。TASK-042 Reports route renders returned Chart DTOs through the existing `chart.bar` view and does not add a production charting-library dependency.
...
```text
Stats dashboard / insight card views beyond TASK-042 Reports
...
saved reporting routes and broad dashboard routes
...
Search Plugin 是内置插件，manifest id 是 `search`。TASK-029 当前交付的是 transient on-demand page scan baseline，不是 persistent indexer、worker、SQLite/FTS 或 native search service。TASK-041 当前交付的是 app-shell Search Dialog 和 shell-owned results route，它消费现有 `search.query` / `search.results` contract，不改变 Search Plugin command/view 注册。
...
Search results 不返回完整页面正文。后续 page edit 会被下一次 `search.query` 看到，因为当前实现每次查询都直接扫描当前 page store，而不是读取持久索引。TASK-041 app shell 以 exact `{ query }` payload 执行 active search-owned `search.query`，把有效返回值复制为 bounded shell-owned route DTO，并且只在确认 result page 仍存在后通过普通 page route state 导航。
