...
tag.tags
habit.enabled
timer.total_tracked_time (deferred after TASK-025)
...
TASK-023 through TASK-025 当前交付的 Metadata UI + Timer Start/segment slice：
...
Full metadata renderer/editor registry, rich field widgets, date picker, estimate editor semantics, Timer tracked-total metadata, save-time scanning/indexing, and app-shell/editor mounting remain deferred. TASK-025 adds Timer's `page.timeline` segment/note slot when a caller mounts that slot; it does not broadly mount page timelines into the app shell.
