...
- Timer Plugin 当前注册 lifecycle commands，并 emit `timer.started` / `timer.paused` / `timer.resumed` / `timer.stopped` events；Time Segment event 仍是 TASK-025 范围；
...
tag.tags
habit.enabled
timer.total_tracked_time (TASK-025+ future)
...
TASK-023/024 当前交付的 Metadata UI + Timer Start slice：
...
- Current built-in contributors are Tag, Task, and Timer controls: Tag add/remove controls keep using Tag commands; Task current fields are read-only; Timer contributes an enabled Start control that executes scoped `timer.start`.

Full metadata renderer/editor registry, rich field widgets, date picker, estimate editor semantics, Time Segment metadata/timeline behavior, save-time scanning/indexing, and app-shell/editor mounting remain deferred.
...
timer.resumed
timer.stopped
timer.time_segment_created (TASK-025 future)
habit.checked
