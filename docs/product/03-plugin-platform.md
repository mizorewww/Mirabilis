...
TASK-023 through TASK-026 当前交付的 Metadata UI、Timer Start/segment 和 Calendar baseline slices：
...
- Calendar contributes registered views, not metadata fields: built-in `calendar` registers `calendar.day` / `calendar.week`, accepts `calendar.time-segments` DTOs, and keeps direct Timer event reads plus Calendar route/feed work deferred.

Full metadata renderer/editor registry, rich field widgets, date picker, estimate editor semantics, Timer tracked-total metadata, save-time scanning/indexing, Calendar app-shell route/feed, and app-shell/editor mounting remain deferred. TASK-025 adds Timer's `page.timeline` segment/note slot when a caller mounts that slot; it does not broadly mount page timelines into the app shell. TASK-026 adds Calendar day/week rendering only when a caller supplies normalized data to the registered view.
