...
Full metadata renderer/editor registry, rich field widgets, date picker, estimate editor semantics, Timer tracked-total metadata, save-time scanning/indexing, Calendar/Habit/Heatmap app-shell route/feed, and app-shell/editor mounting remain deferred. TASK-025 adds Timer's `page.timeline` segment/note slot when a caller mounts that slot; it does not broadly mount page timelines into the app shell. TASK-026 adds Calendar day/week rendering only when a caller supplies normalized data to the registered view. TASK-027 adds Habit commands/filters and Heatmap rendering only when callers provide normalized data to the registered view.
