...
保存后自动扫描新 task block、自动索引、Tag/Timer UI 和富编辑器行为仍属于后续任务。TASK-022 已交付 All Tasks / Today 的 data-only filter execution 和 registered `page.list` 渲染 slice，但它还没有接入保存时自动刷新、全局 saved-filter navigation 或 app-shell filter route。
